...
namespace Pentacorn.Captures
{
    static class LogitechPro9000
    {
        ...
        public static TimeSpan GetExposure(this IKsPropertySet ksPropertySet)
        {
            var ptr = IntPtr.Zero;
            var exposureSupport = new KSPropertySupport();
            ...
            var hr = ksPropertySet.QuerySupported(PROPSETID_LOGITECH_PUBLIC1, ksPropId, out exposureSupport);
            if (hr < 0 || !exposureSupport.HasFlag(KSPropertySupport.Set))
                throw new Exception("Exposure not supported");
            ...
            try
            {
                ...
                Marshal.StructureToPtr(expData, ptr, true);
                int cbReturned;
                int cbExpected = Marshal.SizeOf(expData);
                hr = ksPropertySet.Get(PROPSETID_LOGITECH_PUBLIC1, ksPropId, IntPtr.Zero, 0, ptr, cbExpected, out cbReturned);
                if (hr < 0 || cbReturned != cbExpected)
                    throw new Exception("Exposure problem");
                expData = (KSPROPERTY_LP1_EXPOSURE_TIME_S)Marshal.PtrToStructure(ptr, expData.GetType());
                return TimeSpan.FromMilliseconds(expData.ulExposureTime / 10); // Step size is 100us.
